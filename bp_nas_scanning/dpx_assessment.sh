...
DPX_PATH="${BP_FILM_SCANNING}${DPX_ASSESS}"
ERRORS="${BP_FILM_SCANNING}${CURRENT_ERRORS}"
DPX_REVIEW="${BP_FILM_SCANNING}${DPX_REVIEW}"
...
find "${DPX_PATH}" -maxdepth 3 -mindepth 3 -type d -mmin +30 | -print0 | head -z -5 | while IFS= read -r files; do
