...
cat ${DPX_PATH}temp_dpx_list.txt | while IFS= read -r files; do
    ...
    if [ "$count" -eq 0 ];
      then
        on_global=$(grep "$trim_file" "$global_log" | grep 'Successfully deleted file' | grep $THIS_MONTH)
        if [ -z "$on_global" ]
          then
            retry=$(grep "$trim_file" "$global_log" | grep 'Successfully deleted file' | grep $LAST_MONTH)
            if [ -z "$retry" ]
              then
                skipped=$(grep "$trim_file" "$global_log" | grep 'Skip object' | grep $THIS_MONTH)
                if [ -z "$skipped" ]
                  then
