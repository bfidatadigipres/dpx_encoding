...
DPX_PATH="${QNAP_DIGIOPS}${DPX_COOK}"
MKV_DEST="${QNAP_DIGIOPS}${MKV_ENCODED}"
...
grep '/mnt/' "${MKV_DEST}retry_list.txt" | while IFS= read -r dpx_seq; do
...
cat "${MKV_DEST}retry_list_no_flip.txt" | parallel --jobs 2 "rawcooked -y --all --no-accept-gaps --output-version 2 -s 5281680 ${DPX_PATH}{} -o ${MKV_DEST}mkv_cooked/{}.mkv &>> ${MKV_DEST}mkv_cooked/{}.mkv.txt"
cat "${MKV_DEST}retry_list_image_flip.txt" | parallel --jobs 2 "rawcooked -y --all --no-accept-gaps --output-version 2 -s 5281680 --framemd5 ${DPX_PATH}{} -o ${MKV_DEST}mkv_cooked/{}.mkv &>> ${MKV_DEST}mkv_cooked/{}.mkv.txt"
...
# Check here for RAWcooked_notes.txt, if found add framemd5 output commands
grep '/mnt/' "${MKV_DEST}rawcook_list.txt" | while IFS= read -r dpx_seq; do
  if [ -f "${DPX_PATH}${dpx_seq}RAWcooked_notes.txt" ]
    then
...
cat "${MKV_DEST}rawcook_list_no_flip.txt" | parallel --jobs 2 "rawcooked -y --all --no-accept-gaps -s 5281680 ${DPX_PATH}{} -o ${MKV_DEST}mkv_cooked/{}.mkv &>> ${MKV_DEST}mkv_cooked/{}.mkv.txt"
cat "${MKV_DEST}rawcook_list_image_flip.txt" | parallel --jobs 2 "rawcooked -y --all --no-accept-gaps -s 5281680 --framemd5 ${DPX_PATH}{} -o ${MKV_DEST}mkv_cooked/{}.mkv &>> ${MKV_DEST}mkv_cooked/{}.mkv.txt"
