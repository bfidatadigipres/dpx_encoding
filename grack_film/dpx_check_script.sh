...
MKV_PATH="${GRACK_FILM}${MKV_CHECK}"
MKV_LOG="${GRACK_FILM}${MKV_ENCODED}logs/"
MKV_KILLED="${GRACK_FILM}${MKV_ENCODED}killed/"
MKV_AUTOINGEST="${GRACK_FILM}${AUTOINGEST_VID}"
DPX_PATH="${GRACK_FILM}${DPX_COMPLETE}"
DPX_TO_COOK="${GRACK_FILM}${DPX_COOK}"
FOR_DELETION="${GRACK_FILM}${TO_DELETE}"
...
find "${MKV_PATH}" -name "*.mkv.txt" -mmin +30 | while IFS= read -r log_list; do
