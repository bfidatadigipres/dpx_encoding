...
DPX_PATH="${QNAP_10_DIGIOPS}${DPX_COOK}"
MKV_DEST="${QNAP_10_DIGIOPS}${MKV_ENCODED}"
...
# Sort the temporary_retry_list by part of extension, pass first 20 to retry_list.txt
...
cat "${MKV_DEST}retry_list_no_flip.txt" | parallel --jobs 2 --timeout 259200 "rawcooked -y --all --no-accept-gaps --output-version 2 -s 5281680 ${DPX_PATH}{} -o ${MKV_DEST}mkv_cooked/{}.mkv &>> ${MKV_DEST}mkv_cooked/{}.mkv.txt"
cat "${MKV_DEST}retry_list_image_flip.txt" | parallel --jobs 2 --timeout 259200 "rawcooked -y --all --no-accept-gaps --output-version 2 -s 5281680 --framemd5 ${DPX_PATH}{} -o ${MKV_DEST}mkv_cooked/{}.mkv &>> ${MKV_DEST}mkv_cooked/{}.mkv.txt"
...
find "${DPX_PATH}" -maxdepth 1 -mindepth 1 -type d -name "N_*" | while IFS= read -r folders; do
  ...
  if [ "$count_cooked" -eq 0 ] && [ "$count_queued" -eq 0 ]
    then
      ...
      echo "Failed to write file name to temporary_rawcook_list.txt"
  fi
done
...
cat "${MKV_DEST}rawcook_list_no_flip.txt" | parallel --jobs 2 --timeout 259200 "rawcooked -y --all --no-accept-gaps -s 5281680 ${DPX_PATH}{} -o ${MKV_DEST}mkv_cooked/{}.mkv &>> ${MKV_DEST}mkv_cooked/{}.mkv.txt"
cat "${MKV_DEST}rawcook_list_image_flip.txt" | parallel --jobs 2 --timeout 259200 "rawcooked -y --all --no-accept-gaps -s 5281680 --framemd5 ${DPX_PATH}{} -o ${MKV_DEST}mkv_cooked/{}.mkv &>> ${MKV_DEST}mkv_cooked/{}.mkv.txt"
