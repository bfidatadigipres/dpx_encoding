...
cat "${MKV_DEST}rawcook_list_no_flip.txt" | parallel --jobs 8 --timeout 259200 "rawcooked -y --all --no-accept-gaps -s 9000000 ${DPX_PATH}{} -o ${MKV_DEST}mkv_cooked/{}.mkv &>> ${MKV_DEST}mkv_cooked/{}.mkv.txt"
cat "${MKV_DEST}rawcook_list_image_flip.txt" | parallel --jobs 8 --timeout 259200 "rawcooked -y --all --no-accept-gaps -s 9000000 --framemd5 ${DPX_PATH}{} -o ${MKV_DEST}mkv_cooked/{}.mkv &>> ${MKV_DEST}mkv_cooked/{}.mkv.txt"
