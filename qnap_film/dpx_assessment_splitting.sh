        ...
        then
            ...
            if [ -z "$check" ]
                then
                    ...
                    descriptor=$(mediainfo --Details=1 "${files}/$dpx" | grep "Descriptor" | grep "Luma (Y)")
                    if [ -z "$descriptor" ]
                        then
...
while IFS= read -r line; do
  kb_size=$(du -s "$line")
  log "Size of $line is $kb_size KB. Passing to Python script..."
  parallel --jobs 1 "$PY3_LAUNCH $SPLITTING $kb_size $line 'luma'";
done < "${DPX_PATH}luma_dpx_list.txt"
...
while IFS= read -r line; do
  kb_size=$(du -s "$line")
  log "Size of $line is $kb_size KB. Passing to Python script..."
  parallel --jobs 1 "$PY3_LAUNCH $SPLITTING $kb_size $line 'tar'";
done < "${DPX_PATH}dpx_failures_list.txt"
...
while IFS= read -r line; do
  kb_size2=$(du -s "$line")
  log "Size of $line is $kb_size2 KB. Passing to Python script..."
  parallel --jobs 1 "$PY3_LAUNCH $SPLITTING $kb_size2 $line 'rawcooked'";
done < "${DPX_PATH}dpx_success_list.txt"
