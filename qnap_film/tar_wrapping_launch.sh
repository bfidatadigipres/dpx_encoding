...
# Path to folder
FPATH="${QNAP_FILM}${DPX_WRAP}"
LOGS="${QNAP_FILM}${DPX_SCRIPT_LOG}tar_wrapping_checksum.log"
FLIST="${QNAP_FILM}${TAR_PRES}temp_file_list.txt"
...
grep "/mnt/" "$FLIST" | parallel --jobs 1 "${PY3_ENV} ${DPX_SCRIPTS}qnap_film/tar_wrapping_checksum.py {}"
