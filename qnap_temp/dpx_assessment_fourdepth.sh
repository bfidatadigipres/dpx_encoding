...
DPX_PATH="${QNAP_TEMP}${DPX_ASSESS_FOUR}"
ERRORS="${QNAP_TEMP}${CURRENT_ERRORS}"
DPX_REVIEW="${QNAP_TEMP}${DPX_REVIEW}"
POLICY_PATH="${POLICY_DPX}"
POLICY_PATH2="${POLICY_IMAGE_ORIENTATION}"
PY3_LAUNCH="${PY3_ENV}"
SPLITTING="${SPLITTING_SCRIPT_QNAP_TEMP}"
...
find "${DPX_PATH}" -maxdepth 4 -mindepth 4 -type d -mmin +30 | head -5 | while IFS= read -r files; do
