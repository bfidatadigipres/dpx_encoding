...
grep ^N_ "${MKV_DEST}temporary_retry_list.txt" | rev | sort -n -k10.12 | rev | head -20 > "${MKV_DEST}retry_list.txt"
cook_retry=$(grep ^N_ "${MKV_DEST}retry_list.txt")
log "DPX folder will be cooked using --output-version 2: ${cook_retry}"
...
grep ^N_ "${MKV_DEST}temporary_rawcook_list.txt" | rev | sort -n -k10.12 | rev | head -20 > "${MKV_DEST}rawcook_list.txt"
