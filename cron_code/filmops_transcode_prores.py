...
def make_mov_of_sequence(dpx_folder, fps, output):
    ...
    return ffmpeg + image_settings + glob_config + ffmpeg_input + codec + color_data + fps + output_settings
...
def create_ffmpeg_command(fullpath, output, video_data):
    ...
    sd_16x9 = [
        "-vf",
        "yadif,scale=-1:576:flags=lanczos,pad=1024:576:-1:-1"
    ]
...
def main():
    ...
    for film_item in folder_content:
        ...
        if encode_rule == 'sequence':
            ...
            logger.info("FFmpeg command to create image sequence to MOV: %s", ffmpeg_neat)
        ...
        if encode_rule in ('matroska', 'mov'):
            ...
            if pass_policy == 'pass!':
                ...
                try:
                    ...
                    logger.warning("Shutil move failed for paths:\n%s\n%s", fullpath, COMPLETED)
                    print(err)
                    continue
            else:
                logger.warning("Prores file failed Mediaconch policy: \n%s", pass_policy)
                logger.warning("Deleting ProRes file to allow for repeated attempt: %s", output_fullpath)
                try:
